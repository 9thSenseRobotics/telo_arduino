...
//  accelerate(mySpeed);
  if (mySpeed != 0 && (!checkForFault()))
  {
    ...
    else motorDriver.setSpeedAB(mySpeed - left_motor_bias_default, mySpeed);
    Moving = true;
  }
  else 
  {
  ...
  if (moveTime >= 0) coast();  // Note, if moveTime < 0, we move forever until told to stop
//  decelerate(mySpeed);
}
...
void turnRightForever(int mySpeed) // speed goes from 0 to 255
{
  ...
  if (mySpeed == 0 || checkForFault()) Moving = false;
  else Moving = true;
  timeOutCheck = millis(); 
}
...
void HandleCommand(char* input, int length)
{
  ...
  switch(input[0]) {
    case 't':
      move(speedToGo, 300);  // forward a little
      break;
    case 'f':    
    ...
    case 'F':
      ...
      break;
    case 'g':
      move(-speedToGo, 300);  // backward a little
      break;
    case 'b':
