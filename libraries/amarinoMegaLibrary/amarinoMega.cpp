/*
  amarinoMega.cpp - Arduino Library for Amarino
...
// Private methods
void amarinoMega::processCommand(){
...
amarinoMega::amarinoMega(H_voidFuncPtr err)
{
...
void amarinoMega::registerFunction(void(*userfunction)(uint8_t, uint8_t),uint8_t command){
	intFunc[command-FunctionBufferOffset] = userfunction;
}
void amarinoMega::unregisterFunction(uint8_t command){
	intFunc[command-FunctionBufferOffset] = errorFunc;
}

bool amarinoMega::receive(){
	uint8_t lastByte;
...
void amarinoMega::getBuffer(uint8_t buf[]){
...
void amarinoMega::getString(char string[]){
...
float amarinoMega::getFloat()
{
	return (float)getDouble();
}

int amarinoMega::getArrayLength()
{
...
void amarinoMega::getFloatValues(float values[])
{
...
// not tested yet
void amarinoMega::getDoubleValues(float values[])
{
	getFloatValues(values);
}

// not tested yet
void amarinoMega::getIntValues(int values[])
{
	int t = 0; // counter for each char based array
	int pos = 0;
...
#if defined(ARDUINO) && ARDUINO >= 100
size_t amarinoMega::write(uint8_t b){
	return SERIAL_PORT_BLUETOOTH.print(b);
}
#else
void amarinoMega::write(uint8_t b){
...
void amarinoMega::send(char c ){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.print(c);
	SERIAL_PORT_BLUETOOTH.print(ack);
}

void amarinoMega::send(const char str[]){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.print(str);
	SERIAL_PORT_BLUETOOTH.print(ack);
}
void amarinoMega::send(uint8_t n){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.print(n);
	SERIAL_PORT_BLUETOOTH.print(ack);
}
void amarinoMega::send(int n){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.print(n);
	SERIAL_PORT_BLUETOOTH.print(ack);
}
void amarinoMega::send(unsigned int n){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.print(n);
	SERIAL_PORT_BLUETOOTH.print(ack);
}
void amarinoMega::send(long n){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.print(n);
	SERIAL_PORT_BLUETOOTH.print(ack);
}
void amarinoMega::send(unsigned long n){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.print(n);
	SERIAL_PORT_BLUETOOTH.print(ack);
}
void amarinoMega::send(long n, int base){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.print(n, base);
	SERIAL_PORT_BLUETOOTH.print(ack);
}
void amarinoMega::send(double n){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.print(n);
	SERIAL_PORT_BLUETOOTH.print(ack);
}
void amarinoMega::sendln(void){
	SERIAL_PORT_BLUETOOTH.print(startFlag);
	SERIAL_PORT_BLUETOOTH.println();
	SERIAL_PORT_BLUETOOTH.print(ack);
}

void amarinoMega::flush(){
